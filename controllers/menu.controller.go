...
var menuColllections *mongo.Collection = database.OpenCollection(database.Client, "menu")

func GetMenus() gin.HandlerFunc {
	return func(c *gin.Context) {
		var ctx, cancel = context.WithTimeout(context.Background(), 100*time.Second)
		result, err := menuColllections.Find(context.TODO(), bson.M{})
		defer cancel()
...
func CreateMenu() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		result, insertErr := menuColllections.InsertOne(ctx, menu)
		if insertErr != nil {
			msg := fmt.Sprintf("Menu item was not created")
			c.JSON(http.StatusInternalServerError, gin.H{"erro": msg})
			return
		}
		defer cancel()
...
func UpdateMenu() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if menu.Start_Date != nil && menu.End_Date != nil {
			...
			result, err := menuColllections.UpdateOne(
				ctx,
				filter,
				bson.D{
					{"$set", updateObj},
